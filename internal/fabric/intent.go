...
func ApplyIntent(export *beadspb.Export, ev *gonostr.Event, recipient string) (*beadspb.Export, string, error) {
	...
	if ev.ID == "" || ev.ID != ev.GetID() {
		return nil, "", fmt.Errorf("intent id does not match serialized event")
	}
	ok, err := ev.CheckSignature()
	if err != nil || !ok {
		return nil, "", fmt.Errorf("invalid intent signature")
	}
	if recipient != "" && !eventHasTag(ev, "p", recipient) {
	...
	if method, ok := eventTagValue(ev, "method"); !ok || method != env.Method {
		return nil, "", fmt.Errorf("intent method tag/content mismatch")
	}
	if intentSchema, ok := eventTagValue(ev, "schema"); !ok || intentSchema != schema {
		return nil, "", fmt.Errorf("intent schema tag mismatch")
	}
	switch env.Method {
...
func eventHasTag(ev *gonostr.Event, key, value string) bool {
	got, ok := eventTagValue(ev, key)
	return ok && got == value
}

func eventTagValue(ev *gonostr.Event, key string) (string, bool) {
