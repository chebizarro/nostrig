...
func (s *Service) SyncOnce(ctx context.Context) error {
	...
	digest := modelDigest(local)
	previous, err := s.Store.OutboundDigest(ctx)
	if err != nil {
		return err
	}
	if digest != previous {
		events, err := Encode(local, s.PubKey, time.Now().UTC())
		if err != nil {
			return err
		}
		if len(events) > 0 {
			if _, err := s.Publisher.Publish(ctx, events); err != nil {
				return err
			}
		}
		if err := s.Store.SetOutboundDigest(ctx, digest); err != nil {
			return err
		}
	}
	...
	if len(stateEvents) > 0 {
		local, err = DecodeVerified(stateEvents, s.PubKey)
		if err != nil {
			return err
		}
	}
